...
//RAM	
	...
	scanf("%d",&n_immagini);
	int i,j,pixel,schifo,delta,shift,temp;
...
int n_assert=0;
for(i=0;i<n_immagini;i++)
n_assert=n_assert+(row[i]*col[i]);

k=2;
for(i=0;i<n_immagini;i++){
	k=k+(row[i]*col[i]);
	
	for(j=0;j<row[i]*col[i];j++){
	fprintf(fp,"assert RAM(%d) = std_logic_vector(to_unsigned( %d , 8)) report \"TEST FALLITO (WORKING ZONE). Expected  %d  found\"",k,img_equalizzate[i][j],img_equalizzate[i][j]);
